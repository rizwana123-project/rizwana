...
root.title("ShipNet: Post CNN Model-based Ship Extraction from High Resolution Optical Remotely Sensed Images")
...
def FastRCNN():
    global model
    if os.path.exists('VGGImageNet.h5py'):
        model = tf.keras.models.load_model('VGGImageNet.h5py')
        model.summary()
        ...
        model.save("VGGImageNet.h5py1")
        text.insert(END,"model output can bee seen in black console\n");
        
def extractShip():
    global picture_tensor
    test = filedialog.askopenfilename(initialdir="testimage")
    pathlabel.config(text=test)
...
font = ('times', 18, 'bold')
title = Label(root, text='ShipNet: Post CNN Model-based Ship Extraction from High Resolution Optical Remotely Sensed Images')
...
pathlabel.config(bg='black', fg='wheat')  
pathlabel.config(font=font1)           
pathlabel.place(x=400,y=100)

normal = Button(root, text="Generate Post CNN Model", command=FastRCNN)
